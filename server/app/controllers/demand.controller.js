...
exports.findDemandStatusReport = (req, res) => {
    ...
    return db.sequelize
        ...
        WHERE UNIX_TIMESTAMP(demands.date) BETWEEN UNIX_TIMESTAMP('${from_date}') AND UNIX_TIMESTAMP('${to_date}')`, { type: db.sequelize.QueryTypes.SELECT }
        )
...
exports.findAllTotal = (req, res) => {
    ...
    LEFT JOIN users ON demands.userId = users.id
    LEFT JOIN branches ON users.branchId = branches.id
    WHERE UNIX_TIMESTAMP(date) BETWEEN  UNIX_TIMESTAMP('${fromdate}') AND UNIX_TIMESTAMP('${todate}')`, { type: db.sequelize.QueryTypes.SELECT }
        )
        .then((queues) => res.json(queues))
        .catch((err) => res.status(400).json(err));
};
