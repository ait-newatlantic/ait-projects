...
const WorkDiaryReport = () => {
    ...
    const [from_date, setFromDate] = useState(`${DateFunc.year}-${DateFunc.n}-${DateFunc.d}`);
    ...
    const fetchReports = useCallback(() => {
        ReportService.get_report_from_project(projectId, id, from_date, to_date).then((response) => {
        ...
        <div className="container mx-auto text-center">
            ...
            <div className="flex justify-center space-x-4 mb-4">
                <div className="border border-slate-500">
                    <p className="font-bold border border-slate-600 m-0.5">Thêm nhật ký</p>
                    <form className="px-8 pt-6 text-left space-y-2">
                        <div>
                            <label className="block text-gray-700 text-sm font-bold">
                                Ngày hư hỏng
                            </label>
                            <input className="border border-slate-600" type="date" value={damageDate} onChange={(e) => setDamageDate(e.target.value)} />
                        </div>
                        <div>
                            <label className="block text-gray-700 text-sm font-bold">
                                Nội dung hư hỏng
                            </label>
                            <input className="border border-slate-600" type="text" value={damageContent} onChange={(e) => setDamageContent(e.target.value)} />
                        </div>
                ...
                <div className="border border-slate-500 overflow-auto">
                    <p className="font-bold border border-slate-600 m-0.5">Cập nhật nhật ký</p>
                    <form className="px-8 pt-6 text-left space-y-2 flex flex-wrap items-center">
                        <div className="w-1/5">
                            <label className="block text-gray-700 text-sm font-bold">
                                ID Báo cáo
                            </label>
                            <select className="border border-slate-600 max-w-[177px] w-full" onChange={(e) => setReportId(e.target.value)}>
                        ...
                        <div className="w-1/5">
                            ...
                            <input className="border border-slate-600" type="date" value={fixDate} onChange={(e) => setFixDate(e.target.value)} />
                        </div>
                        <div className="w-1/5">
                            <label className="block text-gray-700 text-sm font-bold">
                                Đội sửa chữa
                            </label>
                            <input className="border border-slate-600" type="text" value={fixCrew} onChange={(e) => setFixCrew(e.target.value)} />
                        </div>
                        <div className="w-1/5">
                            <label className="block text-gray-700 text-sm font-bold">
                                Nội dung sửa chữa
                            </label>
                            <input className="border border-slate-600" type="text" value={fixContent} onChange={(e) => setFixContent(e.target.value)} />
                        </div>
                        <div className="w-1/5">
                            <label className="block text-gray-700 text-sm font-bold">
                                Mã phụ tùng
                            </label>
                            <input className="border border-slate-600" type="text" value={sparePartCode} onChange={(e) => setSparePartCode(e.target.value)} />
                        </div>
                        <div className="w-1/5">
                            <label className="block text-gray-700 text-sm font-bold">
                                Tên phụ tùng & thiết bị
                            </label>
                            <input className="border border-slate-600" type="text" value={sparePartName} onChange={(e) => setSparePartName(e.target.value)} />
                        </div>
                        <div className="w-1/5">
                            <label className="block text-gray-700 text-sm font-bold">
                                ĐVT
                            </label>
                            <select className="border border-slate-600 max-w-[177px] w-full" onChange={(e) => setUnitId(e.target.value)}>
                                <option value="" selected disabled hidden>Chọn</option>
                                {units.map(item => {
                                    return (
                        ...
                        <div className="w-1/5">
                            ...
                            <input className="border border-slate-600" type="text" value={quantity} onChange={(e) => setQuantity(e.target.value)} />
                        </div>
                        <div className="w-1/5">
                            <label className="block text-gray-700 text-sm font-bold">
                                Đơn giá
                            </label>
                            <input className="border border-slate-600" type="text" value={price} onChange={(e) => setPrice(e.target.value)} />
                        </div>
                        <div className="w-1/5">
                            <label className="block text-gray-700 text-sm font-bold">
                                Ghi chú
                            </label>
                            <input className="border border-slate-600" type="text" value={note} onChange={(e) => setNote(e.target.value)} />
                        </div>
                    </form>
                    <button className="mt-2 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button" onClick={handleUpdate}>
                        Save
                    </button>
                </div>
            </div>
            <table className="table-auto border-separate border border-slate-500 w-full">
                <thead>
                    <tr>
                        ...
                        <th className="border border-slate-600">Tên phụ tùng thiết bị & máy móc</th>
