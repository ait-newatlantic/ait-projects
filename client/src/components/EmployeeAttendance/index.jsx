...
const EmployeeAttendance = () => {
    ...
    const handleInputChange = (position) => {
        const updatedCheckedState = checkedState.map((item, index) => {
            return (
                { userId: item.userId, projectId: item.projectId, available: index === position ? !item.available : item.available }
            )
        }
        );
    ...
    const fetchUsers = useCallback(() => {
        UserService.get_users_attendace_from_project(id).then((response) => {
            ...
            const newData = response.data.map(item => {
                return (
                    { userId: item.id, projectId: Number(id), available: false }
                )
            })
    ...
    return (
        <div className="container mx-auto text-center">
            <h3 className="mb-4 uppercase font-bold">Điểm danh nhân sự ngày {formatTime(date)} - {getShipTime()}</h3>
                            ...
                            <tr key={index}>
                                ...
                                <td className="border border-slate-700">
                                    <input type="checkbox" className="form-check-input"
                                        ...
                                        name={item.name}
                                        value={item.index}
                                        checked={checkedState[{ userId: null, projectId: null, available: index }]}
            ...
            <button className="mt-2 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" onClick={handleSubmit}>Save</button>
        </div>
    );
};
