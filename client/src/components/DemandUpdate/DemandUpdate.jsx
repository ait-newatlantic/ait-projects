                                ...
                                <div className="row ">
                                    <div className="col-sm">
                                        ...
                                        <div className="form-control" style={{ overflow: "auto", background: "#e7e7e7" }}>
                                            {!!demands && demands.map(demand => (<div key={demand.id}>{demand.customer_name}</div>))}
                                        </div>
                                    </div>
                                    <div className="col-sm">
                                        Giai đoạn:
                                        {demand_status_id_2 == 9 || demand_status_id_2 == 10 ?
                                            <div>
                                                <div className="form-control" style={{ overflow: "auto", background: "#e7e7e7" }}>
                                                    {!!demands && demands.map(demand => (<div key={demand.id}>{demand.demand_status_name}</div>))}
                                                </div>
                                                <FormHelperText className="text-danger">HOÀN TẤT GIAO DỊCH và GIAO DỊCH THẤT BẠI sẽ không thể thay đổi sang giai đoạn khác</FormHelperText>
                                            </div>
                                            :
                                            <Select className="form-control" id="exampleFormControlSelect1" onChange={onChangeDemandStatusId}>
                                                {!!demand_statuses && demand_statuses.map(demand_status => (
                                                    <option key={demand_status.demand_status_id} value={demand_status.demand_status_id} >{demand_status_name} {'->'} {demand_status.demand_status_name}</option>
                                                ))}
                                            </Select>
                                        }
                                    </div>
                                    <div className="col-sm">
                                        Loại khách hàng :
                                        <div className="form-control" style={{ overflow: "auto", background: "#e7e7e7" }}>
                                            {!!demands && demands.map(demand => (<div key={demand.id}>{demand.customer_type_name}</div>))}
                                        </div>
                                    </div>
                                    <div className="col-sm">
                                        Phương thức liên lạc :
                                        <div className="form-control" style={{ overflow: "auto", background: "#e7e7e7" }}>
                                            {!!demands && demands.map(demand => (<div key={demand.id}>{demand.contact_type_name}</div>))}
                                        </div>
                                    </div>
                                </div>
                                <div className="row ">
                                    {customer_type_id == 3 ?
                                        <div className="col-sm">
                                            ...
                                            <textarea value={demand.demand_opinion} className="form-control" readOnly={true} style={{ background: "#e7e7e7" }} rows="3" key={demand.id} />
                                        ))}
                                        </div>
                                        : <div></div>
                                    }
                                    {contact_type_id == 3 ?
                                        <div className="col-sm">
                                            Địa điểm giao dịch
                                            {!!demands && demands.map(demand => (
                                            <textarea value={demand.demand_meeting} className="form-control" readOnly={true} style={{ background: "#e7e7e7" }} rows="3" key={demand.id} />
                                        ))}
                                        </div> :
                                        <div></div>
                                    }
                                    <div className="col-sm">
                                        Ghi chú:
                                        {!!demands && demands.map(demand => (
                                        <textarea defaultValue={demand.demand_note} className="form-control" rows="3" key={demand.id} onChange={onChangeNote} />
                                    ))}
                                        {demand_status_id == 10 ?
                                            <FormHelperText className="text-danger">Xin vui lòng hãy ghi rõ lí do GIAO DỊCH THẤT BẠI</FormHelperText>
                                            : <div></div>
                                        }
                                    </div>
                                </div>
                            ...
                            <div>
                                ...
                                <div className="row ">
                                    <div className="col-sm">
                                        Người nhập:
                                        <div className="form-control" style={{ overflow: "auto", background: "#e7e7e7" }}>
                                            {!!demands && demands.map(demand => (<div key={demand.id}>{demand.username} - {demand.name}</div>))}
                                        </div>
                                    </div>
                                    <div className="col-sm">
                                        Người gặp khách hàng:
                                        <div className="form-control" style={{ overflow: "auto", background: "#e7e7e7" }}>
                                            {!!demands && demands.map(demand => (<div key={demand.id}>{demand.demand_employee}</div>))}
                                        </div>
                                    </div>
                                    <div className="col-sm">
                            ...
                            <div>
                                ...
                                <div className="row ">
                                    <div className="col-sm">
                                        Model xe:
                                        <div className="form-control" style={{ overflow: "auto", background: "#e7e7e7" }}>
                                            {!!demands && demands.map(demand => (<div key={demand.id}>{demand.car_model_name}</div>))}
                                        </div>
                                    </div>
                                    <div className="col-sm">
                                        Loại xe:
                                        <div className="form-control" style={{ overflow: "auto", background: "#e7e7e7" }}>
                                            {!!demands && demands.map(demand => (<div key={demand.id}>{demand.car_type_name}</div>))}
                                        </div>
                                    </div>
                                    <div className="col-sm">
                                        Số lượng:
                                        <div className="form-control" style={{ overflow: "auto", background: "#e7e7e7" }}>
                                            {!!demands && demands.map(demand => (<div key={demand.id}>{demand.demand_quantity}</div>))}
                                        </div>
                                    </div>
                                    <div className="col-sm">
                                        Màu xe:
                                        <Select className="form-control" onChange={onChangeColor}>
                                            {!!colors && colors.map(color => (
                                                <option key={color.color_id} value={color.color_id} >{color_name} {'->'} {color.color_name}</option>
                                            ))}
                                        </Select>
                                    </div>
                                </div>
