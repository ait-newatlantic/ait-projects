...
const EmployeeAttendanceReport = () => {
    ...
    const fetchAttendace = useCallback(() => {
        AttendanceService.get_attendance_from_project(id, from_date, to_date).then((response) => {
    ...
    const Tick = () => {
        return (
            <img className="h-6" src={logoTick} alt="tick" />
        )
    }
    ...
    const getShipAmount = (username) => {
        const amount = data.filter(item => item.User.username === username).map(item => item.available ? 1 : 0).reduce((partial_sum, a) => partial_sum + a, 0);
        return amount
    }
