...
export default function CustomerUpdate(props) {
    ...
    const FetchData = () => {
        const id = atob(props.match.params.id)
        CustomerService.get_specific_customer(
            id
        )
            .then(response => {
                            ...
                            <div className="row">
                                <div className="col-sm">
                                    ...
                                    <div className="form-control" style={{ overflow: "auto", background: "#e7e7e7" }}>
                                        {!!customers && customers.map(customer => (<div key={customer.id}>{customer.customer_name}</div>))}
                                    </div>
                                </div>
                                <div className="col-sm">
                                    SĐT khách hàng:
                                    <div className="form-control" style={{ background: "#e7e7e7" }}>
                                        {!!customers && customers.map(customer => (<div key={customer.id}>{customer.customer_number}</div>))}
                                    </div>
                                </div>
                            </div>
                            <div className="row">
                                <div className="col-sm">
                                    Khu vực khách hàng:
                                    <div className="form-control" style={{ background: "#e7e7e7" }}>
                                        {!!customers && customers.map(customer => (<div key={customer.id}>{customer.province_name}</div>))}
                                    </div>
                                </div>
                                <div className="col-sm">
                                    Hình thức khách hàng:
                                    <div className="form-control" style={{ background: "#e7e7e7" }}>
                                        {!!customers && customers.map(customer => (<div key={customer.id}>{customer.business_type_name}</div>))}
                                    </div>
                                </div>
                            </div>
                            <div className="row ">
                                <div className="col-sm">
                                    Địa chỉ khách hàng:
                                    <div className="form-control" style={{ background: "#e7e7e7" }}>
                                        {!!customers && customers.map(customer => (<div key={customer.id}>{customer.customer_address}</div>))}
                                    </div>
                                </div>
                            </div>
                        </div>
